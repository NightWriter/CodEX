...
class codexController extends CI_Controller {
    ...
    function codexController ()
    {
        ...
        $this->load->model('main_model','main'); // главная модель
    ...
    function setConfig($config)
    {
         ...
         */
         ...
         $this->export_link              = (isset($config['export_link']))? $config['export_link'] : strtolower($this->controller_name).'/export';
    ...
    function search()
    {
        ...
        if($cnt_keywords != count($fields))
            show_error($this->lang->line('codex_search_keywords_vs_fields'));

        $CI = &get_instance();
        $isset_fields =  $CI->codexforms->iterate('getFieldName');
        ...
        for($i=0; $i < $cnt_keywords; $i++)
        {
            if(!empty($this->codexadmin->form_setup[$fields[$i]]))
            {
                if(strtolower($this->codexadmin->form_setup[$fields[$i]]['class']) == 'dbdropdown'){
                    ...
                    // нельзя искать по полю. Тогда нужно получить обработать поиск.
                    if(empty($isset_fields[$fields[$i]]))
                    {
                        $this->codexforms->objects[$fields[$i]]->prepereSearch($keywords[$i]);
                    }       
                    else 
                        $this->db->like($this->table.'.'.$fields[$i],$keywords[$i]);
                }
    ...
    */
    ...
    function export()
    {
        if(file_exists("./codex/temp/export_{$this->table}.csv"))
            unlink("./codex/temp/export_{$this->table}.csv");
        
        $fields = '';
        $fields_title = '';
        foreach($this->codexadmin->display_fields as $k=>$v)
        {
            if($k  == 'date_cr')
                $fields .= 'FROM_UNIXTIME('.$this->table . '.' . $k . ',"%Y %M %d %H:%i:%s") as date_cr,';
            else
                $fields .= $this->table . '.' . $k . ',';
                //$fields .= 'CONVERT('.$this->table . '.' . $k . ' using cp1251),';
            $fields_title .= $v . ';';
        }
        if(empty($fields))
            $fields = $this->table.'.*';
            
        /*$sql = 'SELECT '.rtrim($fields,',').'
                    
                INTO OUTFILE "'.$_SERVER['DOCUMENT_ROOT'].'/codex/temp/export_'.$this->table.'.csv"
        ...
        force_download("export_{$this->table}.csv", iconv('UTF-8','WINDOWS-1251',rtrim($fields_title,';'))."\n".file_get_contents("./codex/temp/export_{$this->table}.csv"));
        */
        ...
        if($rows->num_rows > 0)
        {
            ...
            //fwrite($fp,iconv('UTF-8','WINDOWS-1251',$fields_title)."\n");
            $lines = iconv('UTF-8','WINDOWS-1251',$fields_title)."\n";
            foreach($rows->result() as $row)
            {
                $line = '';
                foreach($this->codexadmin->display_fields as $k=>$v)
                {
                    $line .= iconv('UTF-8','WINDOWS-1251',$row->{$k}).';';
                }
                $lines .= $line."\n";
                //fwrite($fp,$line."\n");
            }
            //fclose($fp);
            force_download("export_{$this->table}.csv",$lines);
        }
    }
